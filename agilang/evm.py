        ...
        while pc < len(code) and not halted:
            ...
            if opcode == 0x00:  # STOP
                ...
                if name == "ADD": push(pop() + pop())
                    ...
                    a, b = _signed(pop()), _signed(pop()); push(0 if a == 0 else _unsigned_from_signed(int(b / a)))
                elif name == "MOD":
                    a, b = pop(), pop(); push(0 if a == 0 else b % a)
                elif name == "SMOD":
                    a, b = _signed(pop()), _signed(pop()); push(0 if a == 0 else _unsigned_from_signed((abs(b) % abs(a)) * (-1 if b < 0 else 1)))
                elif name == "ADDMOD":
                    a, b, n = pop(), pop(), pop(); push(0 if n == 0 else (a + b) % n)
                elif name == "MULMOD":
                    a, b, n = pop(), pop(), pop(); push(0 if n == 0 else (a * b) % n)
                elif name == "EXP":
                    exponent, base = pop(), pop(); charge(50 * max(1, (exponent.bit_length() + 7) // 8) if exponent else 0); push(pow(base, exponent, WORD_MOD))
                ...
                if name == "LT": a, b = pop(), pop(); push(1 if b < a else 0)
                elif name == "GT": a, b = pop(), pop(); push(1 if b > a else 0)
                elif name == "SLT": a, b = _signed(pop()), _signed(pop()); push(1 if b < a else 0)
                elif name == "SGT": a, b = _signed(pop()), _signed(pop()); push(1 if b > a else 0)
